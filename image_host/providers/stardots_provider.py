...
class StarDotsProvider(ImageHostInterface):
    ...
    def _sync_server_time(self) -> None:
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/openapi/space/list",
                timeout=10,
                verify=False,
            )
    ...
    def _save_records(self):
        ...
        try:
            ...
            logger.warning(f"保存分类记录失败: {str(e)}")
    ...
    def _build_remote_filename(self, image_info: dict[str, str]) -> str:
        ...
        if category and category != self.DEFAULT_CATEGORY:
            encoded_category = self._encode_category(category)
            if encoded_category:
                return f"{encoded_category}{self.CATEGORY_SEPARATOR}{filename}"
        return filename
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if result["success"]:
                            ...
                            if "invalid timestamp" in result.get("message", "").lower():
                                if attempt < max_retries - 1:
                                    logger.warning(
                                        "StarDots timestamp rejected, retrying"
                                    )
                                    time.sleep(retry_delay)
                                    continue
                            if "invalid nonce" in result.get("message", "").lower():
                                if attempt < max_retries - 1:
                                    logger.warning("StarDots nonce rejected, retrying")
                                    time.sleep(retry_delay)
                                    continue
                            # 其他错误，打印消息但继续尝试下一页
                            logger.warning(
                        ...
                        if attempt < max_retries - 1:
                            logger.warning("StarDots list request HTTP error, retrying")
                            time.sleep(retry_delay)
                            continue
                    ...
                    if attempt < max_retries - 1:
                        logger.warning("StarDots list request failed, retrying: %s", e)
                        time.sleep(retry_delay)
                        continue
    ...
    def download_image(self, image_info: dict[str, str], save_path: Path) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                original_name = self._build_remote_filename(image_info)
